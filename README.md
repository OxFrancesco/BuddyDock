...
The BuddyDock CLI only creates exported PNG files. It never modifies app bundles.
The optional icon-pack scripts documented below convert those PNGs, apply verified
custom icons, and restore the icon state that existed before the pack was applied.
...
For a cheap one-icon smoke test, add `--limit 1 --quality low`. Add `--keep-background` to skip Ideogram background removal.
