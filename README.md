...
- `persist`: installs a launchd agent (`ai.buddydock.persist`) that watches `/Applications` and every managed bundle and re-runs `apply` with the last applied manifest whenever one changes, so updates do not lose the icons. The agent runs `bun` directly, so grant that binary App Management access once (System Settings > Privacy & Security > App Management), otherwise writes are denied. Log: `~/Library/Application Support/BuddyDock/persist.log`.
